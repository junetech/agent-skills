...
| `md-to-html` | 마크다운/요청을 4개 모드 중 하나로 변환하고 싶을 때: (1) document: 정적 공간 문서, (2) editor: triage board·flag editor·prompt tuner, (3) deck: 슬라이드 덱, (4) sandbox: 슬라이더 파라미터 샌드박스 | `이거 HTML로 만들어줘`, `plan을 HTML로`, `triage board 만들어`, `slide deck`, `animation sandbox` | 입력 성격에 따라 모드를 자동 선택하여 self-contained 단일 파일 HTML을 원본 옆에 생성합니다. 정적(no-JS)이 기본값; vanilla JS는 interactive 모드에서만. 항상 단일 파일, CDN/네트워크 없음. |
...
- 이 저장소의 공식 공개 skill은 `git-workflow`, `review-pr`, `algorithm-doc-kr`, `md-to-html`, `skill-creator` 다섯 개를 기준으로 관리합니다.
